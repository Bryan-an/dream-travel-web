...
const TravelsPage = () => {
  const { travels, fetchTravels, deleteTravel } = useAppStore();
                  ...
                  ({
                    ...
                    <tr className="border-b border-slate-500" key={id}>
                      ...
                      <td className="px-6 py-4 bg-slate-600">
                        {fecha_salida &&
                          new Date(fecha_salida).toLocaleDateString("en-GB")}
                      </td>
                      <td className="px-6 py-4 bg-slate-700">
                        {fecha_regreso &&
                          new Date(fecha_regreso).toLocaleDateString("en-GB")}
                      </td>
                      ...
                      <td className="px-6 py-4 bg-slate-700">
                        <div className="flex gap-3">
                          <Link href={`/viajes/ver/${id}`}>
                            <EyeIcon />
                          </Link>
                          <Link href={`/viajes/editar/${id}`}>
                            <PencilSquareIcon />
                          </Link>
                          <button
                            onClick={() => {
                              if (
                                confirm(
                                  `¿Estas seguro/a de eliminar este viaje?`
                                )
                              ) {
                                deleteTravel(id);
                              }
